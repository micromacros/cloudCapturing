...
const cryptoFunctions = {
    ...
    // Encrypt
    encryptFile: async (encryptionList, unencryptedFile, callback) => {
        ...
        encrypt(inputFile,encryptionList, (err, encryptedFileStream) => {
            if (err) {
    ...
    // Sign hash
    signFile:async (unencryptedFile, serverPrivateKey, callback) => {
        ...
        sign(inputFile,privKey, (err, signature) => {
            if (err) {
    ...
    //Encrypt Keys
    keyEncryption: async (keyFile, encryptedKeyFile, clientPublicKey) => {
